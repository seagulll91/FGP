...
class SemoAE(BaseModel):
    ...
    def _r6d_norm(self, x):
        rot_num = self.imu_num
        x_shape = x.shape
    ...
    def secondary_motion_gen(self, x, eta=1, acc_gen=True, mask=None):
        if acc_gen:
            ...
            x = self.decoder(x)
            acc, rot = x[:, :, :self.imu_num*3], x[:, :, self.imu_num*3:]
            rot = self._r6d_norm(rot)

        else:
            acc = x[:, :, :self.imu_num*3]
            x = x.detach()
            x = self.encoder(x)
            x = self.dis_normalizer.semo_recon(x1=x, eta=eta, mask=mask)
            x = self.decoder(x)
            rot = x[:, :, self.imu_num*3:]
            rot = self._r6d_norm(rot)
...
class Poser_without_flex(BaseModel):
    def __init__(self,n_input = 4*9, n_output = 10*6):
        super(Poser_without_flex, self).__init__()
        self.lstm = EasyLSTM(n_input=n_input, n_hidden=128, n_output=72 #n_output = 33
                             , n_lstm_layer=2, bidirectional=False, output_type='seq', dropout=0.2)
        self.all_predictor = EasyLSTM(n_input=n_input+72,# n_input = n_input+33
                                       n_hidden=128, n_output=n_output, n_lstm_layer=2, bidirectional=False, output_type='seq', dropout=0.2)
