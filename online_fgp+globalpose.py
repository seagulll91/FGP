...
from GlobalPose_nets.full_GR_OV import Full_GR_OV_10
import os
import time
import numpy as np
import torch
...
from canonicalization_model import IMUDenoiseFlowDiT2
...
OFFLINE_DIR = r'C:\Users\15482\Desktop\ClothData\10IMU\GreenOutfit\G_20260103_zkb\151117_zkb_longJump'
...
def load_flowdit(flow_ckpt_path: str) -> IMUDenoiseFlowDiT2:
...
def stabilize_root_translation(pose_mat: torch.Tensor, tran: torch.Tensor):
    ...
    joint_pos = net.body_model.forward_kinematics(pose_mat.unsqueeze(0),tran = tran.unsqueeze(0))[1][0].detach().cpu()
...
def data_transmit(fps=30):
    ...
    global perf_denoise_ms_accum, perf_pose_ms_accum
    fc = FpsController(set_fps=fps)
    ...
    while RUNNING:
        ...
        if CALIBRATION_DONE == False:
            ...
            if imu_denoiser is not None:
                imu_denoiser.reset()
            print("[Full_GR_OV] RNN state initialized.")
            # 开始上传实时数据
        else:
            if OFFLINE_MODE:
                # 从离线 txt 读取每行数据
            ...
            if ShowRealtimePose:
                ...
                if OFFLINE_PT:
                    ...
                    aM = acc.to(device=device, dtype=torch.float32).view(10, 3)
                    RMB = art.math.normalize_rotation_matrix(
                    ...
                    aM = acc
                    RMB = rot
                    aM = RMB.matmul(aM.unsqueeze(-1)).squeeze(-1)

                # calibrate 输出是 imu_num*(3+9)=imu_num*12
                N = my_server.imu_num  # 一般是11
                # Reuse realtime tensors directly to avoid extra host-side conversions.

                aM = aM.view(10, 3)
                RMB = RMB.view(10, 3, 3)
                # RMB = art.math.normalize_rotation_matrix(RMB)

                # now_t = time.time()
                dt = 1.0 / fps
                if USE_DYNAMIC_GYRO_DT and last_time_cache is not None:
                    dt = float(now_t - last_time_cache)
                    dt = max(dt, 1.0 / (fps * 2))

                if R_prev_cache is None:
                    w_raw = torch.zeros_like(aM)
                else:
                    w_raw = compute_gyro_from_R(R_prev_cache, RMB, dt, angle_eps=GYRO_ANGLE_EPS)

                wM = w_raw # ✅ 再保守裁剪一次
                wM  =RMB.matmul(wM.unsqueeze(-1)).squeeze(-1)

                # if max(aM.view(-1).cpu().numpy())>50:
                #     print(11)
                aM = aM.clamp(-30.0, 30.0) # ✅ 再保守裁剪一次
                # wM = torch.zeros_like(aM)
                # aM = torch.zeros_like(aM)
                # RMB = torch.zeros_like(RMB)
                # RMB = art.math.normalize_rotation_matrix(RMB)
                R_prev_cache = RMB.clone()
                # last_time_cache = now_t
                ...
                with torch.no_grad():
                    pose_mat, tran = net.forward_frame_fast0(aM.to(device), wM.to(device), RMB.to(device))
                    pose_mat = pose_mat.detach().cpu()   # [24,3,3]
                    tran = tran.detach().cpu()           # [3]
