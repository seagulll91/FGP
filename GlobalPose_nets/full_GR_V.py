...
class Full_GR_V(torch.nn.Module):
    dt = 1 / 60
    ...
    def __init__(self):
        ...
        # self.vrnet.load_state_dict(torch.load('data/weights/Tran-V/VR/best_weights.pt'))
        ...
        # for testing
        self.load_state_dict(torch.load('data/weights/Full-GR-V/full/best_weights.pt'))
    ...
    def forward(self, x, fast=True):
        with torch.no_grad():
            ...
            if fast:   # faster approximation
                ...
                x4 = [(torch.cat((RRJ_, pRJ_.detach(), awRB_, gR2_), dim=1), y_[-8:]) for RRJ_, pRJ_, awRB_, gR2_, (x_, y_) in zip(RRJ, pRJ, awRB, gR2, x)]
            else:
                assert False
