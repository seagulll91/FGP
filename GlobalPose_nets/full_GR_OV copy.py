...
class Full_GR_OV_10(torch.nn.Module):
    ...
    def __init__(self):
        ...
        # self.load_state_dict(torch.load('data/weights_only_denoised6.7h/Full-GR-OV/full//weights.pt'))
        # self.load_state_dict(torch.load('data/weights/Full-GR-OV/full/weights.pt'))
    ...
    def forward_frame(self, a, w, R):
        ...
        if not self.is_init:
            ...
            # visualization
            if self.Visualization.enable:
                force_iter = iter(force_vec.reshape(-1, 3)) if np.any(contact) else iter([])
                self.viewer.clear_line(render=False)
                self.viewer.clear_point(render=False)
                self.viewer.update(
                    torch.from_numpy(pose_cur),
                    torch.from_numpy(tran_cur) - self.tran_offset,
                    render=False,
                )
                cjoint_vis = cjoint - self.tran_offset.numpy()
                for i in np.where(contact)[0]:
                    if self.Visualization.show_block and not near_ground[i]:
                        if i < 3 or True:
                            self.scene.add_block(
                                Scene.SupportBlock, cjoint_vis[i] - [0, 0.2, 0],
                                lifetime=120, render=False
                            )
                        else:
                            self.scene.add_block(
                                Scene.GraspBlock, cjoint_vis[i],
                                lifetime=30, render=False
                            )
                    if self.Visualization.show_contact_force:
                        self.viewer.draw_line(
                            cjoint_vis[i],
                            cjoint_vis[i] + self.force_lpf[i](next(force_iter)) * 0.002,
                            [1, 0, 0], 0.02, render=False
                        )
                    if self.Visualization.show_contact:
                        self.viewer.draw_point(
                            cjoint_vis[i], [0, 1, 0], 0.15, render=False
                        )
                if self.Visualization.show_stationary:
                    for i in np.where(stationary & ~contact)[0]:
                        self.viewer.draw_point(
                            cjoint_vis[i], [0.4, 0.4, 1], 0.15, render=False
                        )
                if self.Visualization.show_block:
                    self.scene.update(render=False)
                if self.Visualization.show_residual_force:
                    self.viewer.draw_line_from_joint(
                        0, 0, residual_force[:3] * 0.001, (0, 0, 1), 0.02, render=False
                    )
                if self.Visualization.show_torque:
                    self.viewer.show_torque(
                        0, [1, 2, 4, 5, 16, 17, 18, 19], render=False
                    )
                    tau = M @ qddot + h - torque
                    tau = self.torque_lpf(tau)
                    self.viewer.update_torque(tau[3:], render=False)
