...
class Full_G_OV(torch.nn.Module):
    dt = 1 / 60
    ...
    def __init__(self):
        ...
        # for training
        # self.plnet.load_state_dict(torch.load('data/weights/Pose-G/PL/best_weights.pt'))
        # self.iknet.load_state_dict(torch.load('data/weights/Pose-G/IK/best_weights.pt'))
        # self.vrnet.load_state_dict(torch.load('data/weights/Tran-OV/VR/best_weights.pt'))

        # for testing
        self.load_state_dict(torch.load('data/weights/Full-G-OV/full/weights.pt'))
    ...
    def forward(self, x, fast=True):
        ...
        x1 = [(x_, y_[:15]) for x_, y_ in x]
        x1 = self.plnet(x1)       # pRB

        # IK-s1
        x2 = [torch.cat((RRB_.flatten(1), gR0_, pRB_), dim=1) for RRB_, gR0_, pRB_ in zip(RRB, gR0, x1)]
        x2 = self.iknet.net1(x2)   # pRJ

        # IK-s2
        x3 = [torch.cat((RRB_.flatten(1), gR0_, pRJ_), dim=1) for RRB_, gR0_, pRJ_ in zip(RRB, gR0, x2)]
