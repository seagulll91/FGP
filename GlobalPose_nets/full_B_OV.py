...
class Full_B_OV(torch.nn.Module):
    dt = 1 / 60
    ...
    def __init__(self):
        ...
        # for training
        # self.plnet.load_state_dict(torch.load('data/weights/Pose-B/PL/best_weights.pt'))
        # self.iknet.load_state_dict(torch.load('data/weights/Pose-B/IK/best_weights.pt'))
        # self.vrnet.load_state_dict(torch.load('data/weights/Tran-OV/VR/best_weights.pt'))

        # for testing
        self.load_state_dict(torch.load('data/weights/Full-B-OV/full/weights.pt'))
    ...
    def rnn_initialize(self, init_pose=None, init_vel=None):
        ...
        x1 = torch.cat((pRL,)).to(self.plnet.init_net[0].weight.device)
    ...
    def forward_frame(self, a, w, R):
        ...
        # PL-s1
        x = torch.cat((aRB.ravel(), wRB.ravel(), RRB.ravel()))
        x, self.pl1hc = self.plnet.rnn(x.view(1, 1, -1), self.pl1hc)
        x = self.plnet.linear2(x.squeeze())        # pRB

        # IK-s1
        x = torch.cat((RRB.ravel(), x))
        x, self.ik1hc = self.iknet.net1.rnn(x.view(1, 1, -1), self.ik1hc)
        x = self.iknet.net1.linear2(x.squeeze())   # pRJ
        ...
        # VR-s1
        x = torch.cat((RRJ.ravel().to(aRB.device), pRJ.ravel().to(aRB.device), aRB.ravel(), wRB.ravel(), gR0))
    ...
    def forward(self, x, fast=True):
        ...
        x1 = [(x_[:, :81], y_[:15]) for x_, y_ in x]
        x1 = self.plnet(x1)       # pRB

        # IK-s1
        x2 = [torch.cat((RRB_.flatten(1), pRB_), dim=1) for RRB_, pRB_ in zip(RRB, x1)]
        x2 = self.iknet.net1(x2)   # pRJ

        # IK-s2
        x3 = [torch.cat((RRB_.flatten(1), pRJ_), dim=1) for RRB_, pRJ_ in zip(RRB, x2)]
        ...
        if fast:   # faster approximation
            ...
            x4 = [(torch.cat((RRJ_, pRJ_.detach(), x_[:, :36], x_[:, 81:]), dim=1), torch.zeros_like(y_[-9:]) if torch.isnan(y_[-9]) else y_[-9:]) for RRJ_, pRJ_, (x_, y_) in zip(RRJ, x2, x)]
        else:
            assert False
