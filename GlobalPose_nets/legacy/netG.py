...
class PNPG(torch.nn.Module):
    ...
    def __init__(self):
        ...
        self.plnet.load_state_dict(torch.load('data/weights/PNP-G/PL/best_weights.pt'))
        self.iknet.load_state_dict(torch.load('data/weights/PNP-G/IK/best_weights.pt'))
    ...
    def rnn_initialize(self, init_pose=None):
        if init_pose is None:
            init_pose = torch.eye(3).expand(1, 24, 3, 3)
        else:
            init_pose = init_pose.cpu().view(1, 24, 3, 3)
            init_pose[0, 0] = torch.eye(3)
        pl = self.body_model.forward_kinematics(init_pose, calc_mesh=True)[2].view(6, 3)
        pl = (pl[:5] - pl[5:]).ravel().to(self.plnet.init_net[0].weight.device)
        self.pl1hc = [_.contiguous() for _ in self.plnet.init_net(pl).view(1, 2, self.plnet.num_layers, self.plnet.hidden_size).permute(1, 2, 0, 3)]
        self.ik1hc = None
        self.ik2hc = None
        self.vr1hc = None
        self.vr2hc = None
    ...
    def forward_frame(self, a, w, R):
        ...
        # PL-s1
        x = torch.cat((aRB.ravel(), wRB.ravel(), RRB.ravel(), gR)).unsqueeze(0)
        x, self.pl1hc = self.plnet.rnn(x.unsqueeze(0), self.pl1hc)
        x = self.plnet.linear2(x.squeeze(0))        # pRB

        # IK-s1
        x = torch.cat((RRB.ravel(), gR, x.ravel())).unsqueeze(0)
        x, self.ik1hc = self.iknet.net1.rnn(x.unsqueeze(0), self.ik1hc)
        x = self.iknet.net1.linear2(x.squeeze(0))   # pRJ

        # IK-s2
        x = torch.cat((RRB.ravel(), gR, x.ravel())).unsqueeze(0)
        x, self.ik2hc = self.iknet.net2.rnn(x.unsqueeze(0), self.ik2hc)
        x = self.iknet.net2.linear2(x.squeeze(0))   # RRJ
