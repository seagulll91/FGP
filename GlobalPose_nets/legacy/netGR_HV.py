...
class PNPGR_HV(torch.nn.Module):
    dt = 1 / 60
    ...
    alpha_pd = 0.5     # relaxation in stable PD controller
    ...
    def __init__(self, visualize=False):
        ...
        self.vrnet.load_state_dict(torch.load('data/weights/PNP-GR-HV/VR/best_weights.pt'))

        B = np.array([[self.mu, -self.mu, 0,       0       ],
                      [1,       1,        1,       1       ],
                      [0,       0,        self.mu, -self.mu]]) / np.sqrt(1 + self.mu ** 2)
        self.B = [B, B, B, np.eye(3), np.eye(3)]   # basis of friction cone
        self.lb = [np.zeros(4), np.zeros(4), np.zeros(4), -np.ones(3) * np.inf, -np.ones(3) * np.inf]  # lower bound of force
    ...
    def rnn_initialize(self, init_pose=None, init_vel=None):
        ...
        h, vRR, c = -j[:, :, 1].min().view(1), init_pose[0, 0].t().mm(init_vel.unsqueeze(-1)).squeeze(-1), torch.zeros(5)
    ...
    @torch.no_grad()
    def _explain_residual_force(self, contact, contact_Jacobian, residual_force):
        if np.any(contact):
            J = contact_Jacobian.reshape(5, 3, 75)[contact, :, :6].reshape(-1, 6)
            B = art.math.block_diagonal_matrix_np([self.B[i] for i in range(5) if contact[i]])
            lb = np.concatenate([self.lb[i] for i in range(5) if contact[i]])
            JTB = J.T @ B
            P = csc_array(JTB.T @ JTB + self.beta_extforce * np.eye(JTB.shape[1]))
            q = -JTB.T @ residual_force
            force = solve_qp(P, q, lb=lb, solver='osqp')
    ...
    def forward_frame(self, a, w, R):
        ...
        x = self.vrnet.linear2(x.squeeze())  # height, vRR, stationary_prob

        # get translation estimation
        height, vRR, stationary_prob = x[0].item(), x[1:4].cpu(), x[4:].sigmoid().cpu()
        tran = self.last_cjoint[0] + pose[0].mm(vRR.unsqueeze(-1)).squeeze(-1) * self.dt
        tran[1] = height + self.floor_y
        cjoint = torch.cat((torch.zeros(1, 3), pRJ.mm(pose[0].t())))[self.j_contact, :]
        stationary_weight = (stationary_prob * 5 - 3).clip(0, 1)
        tran = (stationary_weight.unsqueeze(0).mm(self.last_cjoint - cjoint)[0] + self.beta_velocity * tran) / (self.beta_velocity + stationary_weight.sum())
        cjoint += tran
        ...
        if not self.is_init:
            self.is_init = True
            self.physics_model.set_state_R(pose.numpy(), tran.numpy(), np.zeros(75))
        else:
            # get current state
            ...
            cjoint = art.math.lerp(cjoint.numpy(), cjoint_cur, stationary_weight.view(5, 1).numpy())
            ...
            A = np.vstack((np.hstack((np.zeros((72, 3)), np.eye(72))), np.sqrt(self.beta_cjoint) * cJ_cur, np.sqrt(self.beta_torque) * M))
            b = np.concatenate((thetaddotdes, np.sqrt(self.beta_cjoint) * (-cJdot_cur @ qdot + rddotdes), -np.sqrt(self.beta_torque) * h))
            ...
            force, err = self._explain_residual_force(contact, cJ_cur, residual_force)
            for i in np.argsort(cjoint_cur[:, 1]):  # add potential contact from lowest to highest
                if err > 400 and potential_contact[i]:
                    contact[i] = True
                    force_new, err_new = self._explain_residual_force(contact, cJ_cur, residual_force)
                    self.contact_counter[i] = self.contact_counter[i] + 1 if err_new / err < 0.5 else 0
                    # print('\radd contact %d (%d/%d): %.1f->%.1f (%.2f)' % (i, self.contact_counter[i], self.contact_counter_threshold, err, err_new, err_new / err))
                    if self.contact_counter[i] >= 5:
                        force, err = force_new, err_new
                    else:
                        contact[i] = False
                else:
            ...
            cjoint[cjoint[:, 1] < self.floor_y - 0.05, 1] = self.floor_y - 0.05
            ...
            k[contact & near_ground] *= 3
            k[contact & ~near_ground] *= 30
            ...
            if self.visualize:
                ...
                self.viewer.update(torch.from_numpy(pose_cur), torch.from_numpy(tran_cur), render=False)
                # self.viewer.draw_line_from_joint(0, 0, residual_force[:3] * 0.001, (0, 0, 1), 0.02)
                for i in np.where(contact)[0]:
                    if not near_ground[i]:
                        if i < 3:
                            self.scene.add_block(Scene.SupportBlock, cjoint[i] - [0, 0.2, 0], render=False)
                        else:
