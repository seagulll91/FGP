...
class PNPP(torch.nn.Module):
    ...
    def __init__(self):
        ...
        self.plnet.load_state_dict(torch.load('data/weights/PNP-P/PL/best_weights.pt'))
        self.iknet.load_state_dict(torch.load('data/weights/PNP-P/IK/best_weights.pt'))
    ...
    def forward_frame(self, a, w, R):
        ...
        # PL-s1
        x = torch.cat((aPB.ravel(), wPB.ravel(), RPB.ravel(), gR)).unsqueeze(0)
        x, self.pl1hc = self.plnet.rnn(x.unsqueeze(0), self.pl1hc)
        x = self.plnet.linear2(x.squeeze(0))        # pPB

        # IK-s1
        x = torch.cat((RPB.ravel(), gR, x.ravel())).unsqueeze(0)
        x, self.ik1hc = self.iknet.net1.rnn(x.unsqueeze(0), self.ik1hc)
        x = self.iknet.net1.linear2(x.squeeze(0))   # pPJ

        # IK-s2
        x = torch.cat((RPB.ravel(), gR, x.ravel())).unsqueeze(0)
        x, self.ik2hc = self.iknet.net2.rnn(x.unsqueeze(0), self.ik2hc)
        x = self.iknet.net2.linear2(x.squeeze(0))   # RPJ
