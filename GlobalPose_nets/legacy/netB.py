...
class PNPB(torch.nn.Module):
    ...
    def __init__(self):
        ...
        self.plnet.load_state_dict(torch.load('data/weights/PNP-B/PL/best_weights.pt'))
        self.iknet.load_state_dict(torch.load('data/weights/PNP-B/IK/best_weights.pt'))
    ...
    def forward_frame(self, a, w, R):
        ...
        # PL-s1
        x = torch.cat((aRB.ravel(), wRB.ravel(), RRB.ravel())).unsqueeze(0)
        x, self.pl1hc = self.plnet.rnn(x.unsqueeze(0), self.pl1hc)
        x = self.plnet.linear2(x.squeeze(0))        # pRB

        # IK-s1
        x = torch.cat((RRB.ravel(), x.ravel())).unsqueeze(0)
