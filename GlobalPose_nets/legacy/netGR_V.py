...
class PNPGR_V(torch.nn.Module):
    ...
    def __init__(self):
        ...
        self.vrnet.load_state_dict(torch.load('data/weights/PNP-GR-V/VR/best_weights.pt'))
    ...
    def forward_frame(self, a, w, R):
        ...
        x = self.vrnet.linear2(x.squeeze())  # vRR, c
        vRR, self.contact = x[:3], x[3:].sigmoid().cpu()
        vWR = pose[0].mm(vRR.cpu().unsqueeze(-1)).squeeze(-1)
        self.tran += vWR / 60

        return pose, self.tran
