...
class PNPGR(torch.nn.Module):
    ...
    def rnn_initialize(self, init_pose=None):
        ...
        pl = (pl[:5] - pl[5:]).mm(R).ravel().to(self.plnet.init_net[0].weight.device)
        g = -R[1].to(self.plnet.init_net[0].weight.device)
