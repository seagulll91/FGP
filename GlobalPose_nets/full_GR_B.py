...
class Full_GR_B(torch.nn.Module):
    dt = 1 / 60
    ...
    def __init__(self):
        ...
        # for training
        # weights = torch.load('data/weights/Full-GR-OV/full/weights.pt')
        # self.load_state_dict({k: v for k, v in weights.items() if k[:2] != 'vr'}, strict=False)
        # self.vrnet.load_state_dict(torch.load('data/weights/Tran-B/VR/best_weights.pt'))
        # for p in self.plnet.parameters():
        #     p.requires_grad = False
        # for p in self.iknet.parameters():
        #     p.requires_grad = False

        # for testing
        self.load_state_dict(torch.load('data/weights/Full-GR-B/full/best_weights.pt'))
    ...
    def rnn_initialize(self, init_pose=None, init_vel=None):
        ...
        x2 = torch.cat((vRR, c)).to(self.vrnet.init_net[0].weight.device)
    ...
    def forward_frame(self, a, w, R):
        ...
        aRB = a.cpu().mm(pose[0])
        wRB = w.cpu().mm(pose[0])
        x = torch.cat((RRJ.ravel(), pRJ.ravel(), aRB.ravel(), wRB.ravel())).to(gR2.device)
        x, self.vr1hc = self.vrnet.rnn(x.view(1, 1, -1), self.vr1hc)
        x = self.vrnet.linear2(x.squeeze())  # vRR, stationary_prob

        # get translation estimation
        vRR, stationary_prob = x[:3].cpu(), x[3:].sigmoid().cpu()
    ...
    def forward(self, x, fast=True):
        with torch.no_grad():
            ...
            if fast:   # faster approximation
                ...
                x4 = [(torch.cat((RRJ_, pRJ_.detach(), awRB_), dim=1), y_[-8:]) for RRJ_, pRJ_, awRB_, (x_, y_) in zip(RRJ, pRJ, awRB, x)]
            else:
                assert False
        x4 = self.vrnet(x4)   # vRR, stationary_prob
        return x4
