...
def from_to_rotation_matrix(from_vector: torch.Tensor, to_vector: torch.Tensor):
    ...
    """
    from_vector = normalize_tensor(from_vector.reshape(-1, 3))
...
def axis_angle_to_rotation_matrix(a: torch.Tensor):
    ...
    c, s = angle.cos().reshape(-1, 1, 1), angle.sin().reshape(-1, 1, 1)
    r = c * i_cube + (1 - c) * torch.bmm(axis.reshape(-1, 3, 1), axis.reshape(-1, 1, 3)) + s * hat(axis)
    return r
